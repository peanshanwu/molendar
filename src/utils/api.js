...
export async function fetchMovie(movie_id) {
  const response = await fetch(
    `https://api.themoviedb.org/3/movie/${movie_id}?api_key=12e9a6484e6de743210098e375735e0d&language=en-US&append_to_response=videos`
  );
...
export async function fetchUpcomingNowPlayingMovies() {
  const [upcoming, nowPlaying] = await Promise.all([
    fetch(
      "https://api.themoviedb.org/3/movie/upcoming?api_key=12e9a6484e6de743210098e375735e0d&language=en-US&append_to_response=videos"
    ),
    fetch(
      "https://api.themoviedb.org/3/movie/now_playing?api_key=12e9a6484e6de743210098e375735e0d&language=en-US&append_to_response=videos"
    ),
  ]);
...
export async function fetchMultiMovies(movieArr) {
  ...
  filterResult.forEach((movie) => {
    movieIdArr.push(
      fetch(
        `https://api.themoviedb.org/3/movie/${movie.movie_id}?api_key=12e9a6484e6de743210098e375735e0d&language=en-US&append_to_response=videos`
      )
    );
  });
