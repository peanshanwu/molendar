...
const PersonalCalendar = ({ selectDay, setSelectDay, uid, userList, myCalendarMovies, calendarMoviesInfo }) => {
										...
										>
	...
	
											{
												...
												&&
													<MovieContainer>
													<Mask />
													<MovieName>{ calendarMoviesInfo.length !==0 && getMovieInfo(getMovieId(date.date)).original_title }</MovieName>
													<Poster src={ calendarMoviesInfo.length !==0 && `https://image.tmdb.org/t/p/w500/${getMovieInfo(getMovieId(date.date)).poster_path}`} alt="" />
												</MovieContainer>
											}
										</td>
									)
